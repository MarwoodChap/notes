---
title: Dramatis Personae
share: true
category: sundered-reach
tags:
  - campaign
  - reference
---

# Dramatis Personae

*A roster, not a history. One line each. Detail lives on the faction pages.*

---

## House Crosius

| | | |
|---|---|---|
| **Lord-Captain Domenico Crosius** | Warrant-holder | Navy officer who inherited a dynasty he never trained for, and keeps refusing to be cruel about it. → [[./House Crosius|House Crosius]] |
| **Inquisitor Hadrian Thule** | Ordo Scriptorum | Came to check a filing discrepancy four years ago. Commands nothing, and therefore commands everything. → [[./House Crosius|House Crosius]] |
## The Red Knights of Oblivion

|                                              |              |                                                                                                 |
| -------------------------------------------- | ------------ | ----------------------------------------------------------------------------------------------- |
| **Prince Verux the Unholy**                  | Demon Prince | Daemon Prince. Ascended by killing Emperor's Children, and would very much like to do it again. |
| **Kharn the Betrayer**                       |              | A raging storm of slaughter. Here. Somehow.                                                     |
| **Ba'rule, Eighth Lord of the Skull Throne** |              | Bound by pact aboard the _Eviscerator Rex_. Owed something. Not patient.                        |
## The unaligned

These belong to nobody. Anyone may use them.

| | | |
|---|---|---|
| **Saint Ysolde of Vantry** | Dead, contested | Farm girl who held the pass for three days. Venerated by tens of millions, recognised by no one — [[Vantry|Vantry]] cannot ratify a saint with Terra out of reach. |
| **House Sabaeus** | Navigators, [[Threnody|Threnody]] | The only people who can actually read [[./The Storm|The Storm]]. They have an assessment. They have not published it. |
| **The Concordant bloc** | [[Concordia|Concordia]] | Hold every world. Abandon nothing. |
| **The Retrenchment bloc** | [[Concordia|Concordia]] | Pull back to the inner five and save what can be saved. |
| **House Corvane** | Knights, [[Corvane's Rest|Corvane's Rest]] | Oath-bound to Crosius for six thousand years. Down to a handful of working machines, because [[Skarn|Skarn]] can no longer service them. |
| **The forge-clave of [[Skarn|Skarn]]** | Adeptus Mechanicus | Cut off from Mars, improvising. They know what that is called. |
| *"the Kill-Team"* | Unconfirmed | Persistent reports of Adeptus Astartes operating in the Reach without Chapter support. No one has produced one. |

## Other factions

*(Players: add your leaders here — name, one line, link to your faction page.)*

| | | |
|---|---|---|
| | [[Faction — Aeldari|Faction — Aeldari]] | |
| | [[Faction — World Eaters|Faction — World Eaters]] | |
| | [[Faction — Emperor's Children|Faction — Emperor's Children]] | |

---

## Adding someone

Anyone may add a character. Two rules:

1. **One line here, detail on your own page.** This roster is for scanning before a game.
2. **If they're on the table, they can die.** Except yours — nobody's character dies without their player agreeing.
